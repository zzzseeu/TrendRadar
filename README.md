...
周一以 `published_at` 严格筛选上一自然周的候选内容，并使用全局 `0.5` 分数阈值。周报有四个独立模块：

- 水稻育种政策最多 20 条，仅收水稻相关政策。
- 水稻产业时事动态最多 20 条，仅收含实质水稻产业信息的动态。
- 农业育种科研文献最多 20 条，水稻优先，其他作物研究可作为借鉴补充。
...
水稻政策 20 + 水稻产业动态 20 + 水稻优先科研 20 + 独立农业气象 →
一个 PDF → 企业微信文件消息
```
