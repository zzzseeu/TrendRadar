...
**目标：** 将周一周报首次运行调整为 12:10，并在失败时于 12:30、13:00、13:30 自动重试；保持周二至周日 10:00 静默采集。

**架构：** Docker Cron 负责精确唤醒进程，时间线负责判定周报执行窗口。已有周成功检查点负责让成功后的重试触发在业务处理前短路，因此后续三个时刻不是重复推送。所有部署入口、示例和文档使用同一时间表，避免只改一层造成任务被调度器拒绝。
...
将旧的周一 `10:30—12:00` 和旧 Cron 示例替换为“12:10 首次运行，12:30、13:00、13:30 仅在尚未成功时重试”，并保持周二至周日 `10:00` 静默采集描述。
