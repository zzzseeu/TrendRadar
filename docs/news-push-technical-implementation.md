...
系统在周二至周日每天北京时间 10:00 静默采集数据。每周一以 `published_at` 严格筛选上一自然周的内容，并应用全局 `0.5` 分数阈值。周报由四个独立模块组成：水稻育种政策、水稻产业时事动态、农业育种科研文献各最多 20 条，以及不占新闻名额的农业气象模块。政策和产业动态只收水稻，科研文献水稻优先、其他作物可作借鉴。最后通过企业微信 `upload_media` 上传并仅发送一个 PDF 文件。
